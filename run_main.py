...
from utils.tools import del_files, EarlyStopping, adjust_learning_rate, vali, load_content, test_res
...
parser.add_argument('--description', type=str, default="The illness dataset describes the ratio of patients with influenza illness to the number of patients. It includes weekly data from the U.S. Centers for Disease Control and Prevention from 2002 to 2021.", help='description')
...
parser.add_argument('--llm_model', type=str, default='GPT2', help='LLM model') # Qwen, LLAMA, GPT2, BERT
parser.add_argument('--llm_dim', type=int, default='768', help='LLM model dimension')# Qwen:1536 Qwen:3584 LLama7b:4096; GPT2-small:768; BERT-base:768
# parser.add_argument('--description', type=str, default='The illness dataset describes the ratio of patients with influenza illness to the number of patients. It includes weekly data from the U.S. Centers for Disease Control and Prevention from 2002 to 2021.', help='description')
# parser.add_argument('--patch_lens', type=int, default=[8,4,2], help='patch_lens')
...
parser.add_argument('--patch_size', type=int, default=24, help='the patch size')
...
# Mamba
parser.add_argument('--e_fact', type=int, default=8, help='expand factor parameter of Mamba')
...
for ii in range(args.itr):
    ...
    if args.model == 'SLAN':
        model = SLAN.Model(args).bfloat16() 
    ...
    #结果保存
    # folder_path = './visresults/' + setting + '/'
    ...
    for epoch in range(args.train_epochs):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(train_loader),disable=True):
            ...
            if args.use_amp:
                with torch.cuda.amp.autocast():
                    ...
                    outputs = outputs[:, -args.pred_len:, f_dim:].to(torch.bfloat16)
                ...
                f_dim = -1 if args.features == 'MS' else 0
                outputs = outputs[:, -args.pred_len:, f_dim:].to(torch.bfloat16)
                batch_y = batch_y[:, -args.pred_len:, f_dim:].to(torch.bfloat16)
                loss = criterion(outputs, batch_y)
                train_loss.append(loss.item())
            ...
            if (i + 1) % 100 == 0:
                ...
                accelerator.print('\tspeed: {:.4f}s/iter; left time: {:.2f}hours'.format(speed, left_time/60/60))
                iter_count = 0
                time_now = time.time()
        ...
        vali_loss, vali_mae_loss, vali_mape_loss = vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric, mape_metric)
        test_loss, test_mae_loss, test_mape_loss = test_res(args, accelerator, model, test_data, test_loader, criterion, mae_metric, mape_metric)
        accelerator.print(
            "Epoch: {0} | Train Loss: {1:.7f} Vali Loss: {2:.7f} Test MSE Loss: {3:.7f} MAE Loss: {4:.7f} ".format(
                epoch + 1, train_loss, vali_loss, test_loss, test_mae_loss))

        early_stopping(vali_loss, model, path)
        if early_stopping.early_stop:
            accelerator.print("Early stopping")
            # with open("result_SST_forecast.txt", 'a') as f:
            #     f.write(setting + "  \n")
            #     f.write('rmse:{}, mae:{}, mape:{}\n'.format(np.sqrt(test_loss), test_mae_loss, test_mape))
            #     f.write('\n')            
